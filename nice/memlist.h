 ...
 * (C) 2022 Pexip AS
 *
 * The contents of this file are subject to the Mozilla Public License VersioVn
...
typedef NiceMemoryBufferRef* (*nice_memory_buffer_get)(MemlistInterface **ml_interface, gsize size);
/* Return a memory buffer to the provider */
typedef void (*nice_memory_buffer_return)(MemlistInterface **ml_interface, NiceMemoryBufferRef* buffer);
/* Get a pointer to the contents (i.e. bytes) of the memory buffer */
typedef char* (*nice_memory_buffer_contents)(MemlistInterface **ml_interface, NiceMemoryBufferRef* buffer);
/* Get the size of the memory buffer */
typedef gsize (*nice_memory_buffer_size)(MemlistInterface **ml_interface, NiceMemoryBufferRef* buffer);
/* Reduce the size of the memory buffer */
typedef void (*nice_memory_buffer_resize)(MemlistInterface **ml_interface, NiceMemoryBufferRef* buffer, gsize new_size);
